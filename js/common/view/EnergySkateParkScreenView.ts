...
export default class EnergySkateParkScreenView extends ScreenView {
  ...
  public constructor( model: EnergySkateParkModel, tandem: Tandem, providedOptions?: EnergySkateParkScreenViewOptions ) {
    ...
    if ( options.showToolbox ) {
      ...
      this.stopwatchNode = new StopwatchNode( model.stopwatch, {
        ...
        dragListenerOptions: {
            ...
            if ( this.toolboxPanel!.visible && this.stopwatchNode!.globalBounds.intersectsBounds( this.toolboxPanel!.globalBounds ) ) {
    ...
    // AlignBox wouldn't work here because fixedRight is computed dynamically in layout() based on
    // the visible bounds with padding, and we need the VBox to float to the right edge specifically.
